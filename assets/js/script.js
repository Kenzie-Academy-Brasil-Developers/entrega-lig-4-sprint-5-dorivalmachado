...
function showResult(result){
    ...
    let gameDiv = document.getElementClassName("game")
    gameDiv.appendChild(divResult)
}
...
const insertDisk = (selectedColumn, player) => {
    ...
    for (let i = 0; i < column.length; i++) {
        if (column[i] === 'X') {
            ...
            if(checkVictory((Number(columnIndex) - 1),i)){
                showResult("victory")
            }
            else if(draw()){
                showResult("defeat")
